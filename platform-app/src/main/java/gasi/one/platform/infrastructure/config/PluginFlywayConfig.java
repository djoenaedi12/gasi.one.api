...
public class PluginFlywayConfig {
    ...
    private static final String PLUGIN_FLYWAY_INITIALIZER = "pluginFlywayInitializer";
    ...
    /**
     * Creates an initializer that executes Flyway migrations from all registered
     * locations using a composite plugin classloader.
     *
     ...
     * @return initializer that performs the combined Flyway migration
     */
    @Bean(PLUGIN_FLYWAY_INITIALIZER)
    public InitializingBean pluginFlywayInitializer(
        ...
        return () -> {
            ...
            LOG.info("[Flyway] Migration complete. {} script(s) executed.", applied);
        };
    }

    /**
     * Ensures JPA initialization waits for platform and plugin migrations.
     *
     * @return post-processor that wires the entity manager factory dependency
     */
    @Bean
    public static BeanFactoryPostProcessor pluginFlywayBeforeJpa() {
        return beanFactory -> {
            if (!beanFactory.containsBeanDefinition(ENTITY_MANAGER_FACTORY)
                    || !beanFactory.containsBeanDefinition(PLUGIN_FLYWAY_INITIALIZER)) {
                return;
            }

            BeanDefinition definition = beanFactory.getBeanDefinition(ENTITY_MANAGER_FACTORY);
            definition.setDependsOn(appendDependsOn(definition.getDependsOn(), PLUGIN_FLYWAY_INITIALIZER));
        };
    }
    ...
    private static String[] appendDependsOn(String[] dependsOn, String beanName) {
        ...
        result[dependsOn.length] = beanName;
        return result;
    }
