...
public interface ResourceRepositoryHook<D extends BaseModel> {
    ...
    default GenericFilter applyRecordRules(String resourceType, GenericFilter filter) {
        return filter;
    }
    ...
    static <D extends BaseModel> ResourceRepositoryHook<D> composite(
        ...
        return new ResourceRepositoryHook<>() {
            ...
            @Override
            public GenericFilter applyRecordRules(String resourceType, GenericFilter filter) {
                GenericFilter next = filter;
                for (ResourceRepositoryHook<D> hook : hooks) {
                    next = hook.applyRecordRules(resourceType, next);
                }
                return next;
            }
